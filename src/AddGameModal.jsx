...
export default function AddGameModal({ isOpen, onClose, onAddGame }) {
  ...
  const handleSubmit = (event) => {
    ...
    onAddGame({ id: Date.now(), gameId: normalizedGameId, matchup, sport, secondsToKickoff: Number(secondsToKickoff), fairSpread: Number(fairSpread), actualSpread: Number(actualSpread) });
    setMatchup('');
    setGameId('');
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0, 0, 0, 0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', zIndex: 10 }}>
      <div style={{ background: '#1c2541', border: '1px solid #48cae4', borderRadius: '8px', padding: '25px', width: '100%', maxWidth: '480px' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '12px' }}>
          ...
          <label style={labelStyle}>Game ID<input value={gameId} onChange={(event) => setGameId(event.target.value)} style={inputStyle} placeholder="Provider event ID" /></label>
          ...
          <label style={labelStyle}>Actual spread<input required type="number" step="0.5" value={actualSpread} onChange={(event) => setActualSpread(event.target.value)} style={inputStyle} /></label>
